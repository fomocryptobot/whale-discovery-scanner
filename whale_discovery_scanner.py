...
print("🚀 FCB WHALE DISCOVERY SCANNER STARTING...", flush=True)
...
target_tokens = {
    'ethereum': {
        ...
        # Major meme/pump tokens - FIXED CONTRACT ADDRESSES
        ...
        # Removed DOGE, WIF, BONK - these are not ERC-20 or incorrect addresses
        
        # Stablecoins (high volume whale activity) - FIXED USDC ADDRESS
        'USDC': {'contract': '0xa0b86a33e604ee5c9611c0516c50f60e6966f945', 'decimals': 6},
        ...
        # Layer 2 / Scaling tokens - FIXED OP ADDRESS (this is Arbitrum, not Optimism)
...
class WhaleDiscoveryScanner:
    ...
    def get_next_fcb_session_time(self):
        ...
        next_session_time = datetime.combine(
            next_session_date, 
            datetime.min.time().replace(hour=next_session_hour),
            timezone.utc
        )
    ...
    def get_current_whale_count(self):
        ...
        try:
            ...
            print(f"⚠️ Failed to get whale count: {e}", flush=True)
            return 0
        finally:
            conn.close()
    ...
    def get_token_price(self, contract_address):
        ...
        try:
            ...
            print(f"⚠️ Price fetch failed for {contract_address}: {e}", flush=True)
        return 0
    ...
    def get_current_block(self):
        ...
        try:
            ...
            print(f"⚠️ Failed to get current block: {e}", flush=True)
            return 0
    
    def get_block_timestamp(self, block_number):
        """Get timestamp for a specific block"""
        try:
            url = f"https://api.etherscan.io/api?module=proxy&action=eth_getBlockByNumber&tag=0x{block_number:x}&boolean=false&apikey={ETHERSCAN_API_KEY}"
            response = requests.get(url, timeout=10)
            if response.status_code == 200:
                data = response.json()
            ...
            print(f"⚠️ Failed to get block timestamp: {e}", flush=True)
        return datetime.now(timezone.utc)
    
    def discover_new_whales(self):
        """Discover new whale wallets in the last 6 hours"""
        print(f"\n🔍 WHALE DISCOVERY SCAN #{self.scan_count + 1}", flush=True)
        print(f"⏰ {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}", flush=True)
        print("=" * 60, flush=True)
        
        current_block = self.get_current_block()
        if current_block == 0:
            print("❌ Cannot get current block", flush=True)
            return 0, 0
        ...
        print(f"📊 Scanning blocks {start_block:,} to {current_block:,}", flush=True)
        print(f"🕐 Looking back {SCAN_HOURS_BACK} hours (since last FCB session)", flush=True)
        print(f"🎯 Preparing whale intelligence for FCB session", flush=True)
        ...
        for token_symbol, token_data in target_tokens['ethereum'].items():
            print(f"\n🎯 Scanning {token_symbol} for new whales...", flush=True)
            ...
            if token_price == 0:
                print(f"⚠️ No price data for {token_symbol}, skipping...", flush=True)
                continue
            
            print(f"💰 {token_symbol} price: ${token_price:.8f}", flush=True)
            ...
            try:
                ...
                if data.get('status') != '1':
                    ...
                    print(f"⚠️ API error for {token_symbol}: {error_msg}", flush=True)
                ...
                print(f"📈 Found {len(transfers):,} {token_symbol} transfers", flush=True)
                    ...
                    try:
                        ...
                        if amount_usd >= WHALE_THRESHOLD:
                            ...
                            if len(topics) >= 3:
                                ...
                                block_timestamp = self.get_block_timestamp(block_number)
                                ...
                                if self.save_whale_transaction(whale_transaction):
                                    ...
                                    if token_whales <= 5:  # Show first 5 whales per token
                                        print(f"🐋 NEW WHALE: {whale_address[:10]}... ${amount_usd:,.2f} {token_symbol} {activity_type}", flush=True)
                    
                    except Exception as e:
                        continue
                
                if token_whales > 0:
                    print(f"✅ {token_symbol}: {token_whales} new whales, ${token_volume:,.2f} volume", flush=True)
                    new_whales_found += token_whales
                    total_volume += token_volume
                else:
                    print(f"😴 {token_symbol}: No new whale activity", flush=True)
                
            except Exception as e:
                print(f"❌ Failed to scan {token_symbol}: {e}", flush=True)
    ...
    def generate_discovery_report(self, new_whales, volume, next_session_time):
        ...
        print(f"\n" + "=" * 80, flush=True)
        print(f"📊 FCB PRE-SESSION WHALE DISCOVERY COMPLETE", flush=True)
        print(f"=" * 80, flush=True)
        print(f"🐋 New whales discovered: {new_whales}", flush=True)
        print(f"💰 New volume analyzed: ${volume:,.2f}", flush=True)
        print(f"📈 Total whales in database: {current_whale_count}", flush=True)
        print(f"🎯 Next FCB session: {next_session_time.strftime('%H:%M UTC (%Y-%m-%d)')}", flush=True)
        print(f"⏰ Time until session: {int((next_session_time - datetime.now(timezone.utc)).total_seconds() / 60)} minutes", flush=True)
        
        self.total_whales_discovered += new_whales
        
        if new_whales > 0:
            print(f"🚀 SUCCESS! Fresh whale intelligence ready for FCB session", flush=True)
        else:
            print(f"😴 No new whales since last session - database stable", flush=True)
        
        # Calculate next discovery time
        next_discovery_time, next_next_session = self.get_next_fcb_session_time()
        print(f"🔍 Next whale discovery: {next_discovery_time.strftime('%H:%M UTC (%Y-%m-%d)')}", flush=True)
    
    async def run_discovery_loop(self):
        """Main discovery loop - runs 30 minutes before each FCB session"""
        print("🚀 FCB WHALE DISCOVERY SCANNER - PRE-SESSION INTELLIGENCE", flush=True)
        print("=" * 80, flush=True)
        print(f"🕐 FCB Sessions: {', '.join([f'{h:02d}:00 UTC' for h in FCB_SESSION_HOURS])}", flush=True)
        print(f"🔍 Discovery timing: 30 minutes before each session", flush=True)
        print(f"🎯 Whale threshold: ${WHALE_THRESHOLD:,}+", flush=True)
        print(f"📊 Lookback period: {SCAN_HOURS_BACK} hours per scan", flush=True)
        print(f"🪙 Target tokens: {len(target_tokens['ethereum'])} tokens", flush=True)
        ...
        while True:
            try:
                ...
                if sleep_seconds > 0:
                    print(f"\n⏰ Next discovery: {next_discovery_time.strftime('%H:%M:%S UTC (%Y-%m-%d)')}", flush=True)
                    print(f"🎯 For FCB session: {next_session_time.strftime('%H:%M:%S UTC')}", flush=True)
                    print(f"💤 Sleeping for {sleep_seconds/3600:.1f} hours...", flush=True)
                    await asyncio.sleep(sleep_seconds)
                
                self.scan_count += 1
                
                print(f"\n🚨 FCB PRE-SESSION WHALE DISCOVERY #{self.scan_count}", flush=True)
                print(f"🕐 Discovery time: {datetime.now(timezone.utc).strftime('%H:%M:%S UTC')}", flush=True)
                print(f"🎯 FCB session starts: {next_session_time.strftime('%H:%M:%S UTC')} (in 30 minutes)", flush=True)
                ...
                print("\n🛑 FCB Discovery scanner stopped by user", flush=True)
                break
            except Exception as e:
                print(f"⚠️ Discovery error: {e}", flush=True)
                print("🔄 Retrying in 30 minutes...", flush=True)
...
def main():
    ...
    try:
        ...
        print("\n👋 FCB Whale Discovery Scanner stopped", flush=True)
